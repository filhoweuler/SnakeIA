...
def sigmoid(x, derivative=False):
    return sigmoid(x)*(1.0-sigmoid(x)) if derivative else 1.0/(1.0+np.exp(-x))
...
if __name__ == "__main__":
    ...
    epochs = 0
    diff = 5
    while(diff > 0.07 and epochs < 1000):
        epochs -= 1
        past_diff = diff
        ...
        with open('w1_' + str(epochs), 'w') as out:
            json.dump(w1, out)

        with open('w2_' + str(epochs), 'w') as out:
            json.dump(w2, out)